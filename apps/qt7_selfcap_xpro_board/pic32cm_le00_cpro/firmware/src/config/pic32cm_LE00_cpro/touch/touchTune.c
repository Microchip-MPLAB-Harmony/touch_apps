...
static uint8_t proj_config[PROJECT_CONFIG_DATA_LEN] = {PROTOCOL_VERSION, (uint8_t) saml1x_pic32cmle, (uint8_t) TECH, (DEF_NUM_CHANNELS),
...
void UART_Write(uint8_t data) {
	...
	txData = data;
	if (SERCOM3_USART_Write(&txData, 1)) {
...
void touchTuneInit(void) {
    ...
    if(SERCOM3_USART_Read((void *) &rxData, 1)) {
