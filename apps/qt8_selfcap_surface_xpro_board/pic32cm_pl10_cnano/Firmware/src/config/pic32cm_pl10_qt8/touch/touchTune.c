...
*******************************************************************************/
#define NO_OF_CONFIG_FRAME_ID	 (7u)
...
#if SURFACE_MODULE_OUTPUT == 1u
#define CONFIG_5_PTR ((uint8_t*) (&qtm_surface_cs_config1.start_key_h))
#define CONFIG_5_LEN ((uint16_t)  11u)
#endif

#if GESTURE_MODULE_OUTPUT == 1u
#define CONFIG_6_PTR ((uint8_t*) (&qtm_gestures_2d_config.tapReleaseTimeout))
#define CONFIG_6_LEN ((uint16_t)  13u)
#endif

/*******************************************************************************
 * Debug Data Configuration
*******************************************************************************/
#define OUTPUT_MODULE_CNT 4u
...
#if SURFACE_MODULE_OUTPUT == 1u
#define DATA_2_PTR 			((uint8_t*)&runtime_surface_data_arr.status)
#define DATA_2_ID 			(DEBUG_MASK + SURFACE_DEBUG_DATA_ID)
#define DATA_2_LEN			(uint16_t) (sizeof(tuneSurfaceData_t))
#define DATA_2_REPEAT 		2u
#define DATA_2_FRAME_LEN 	(uint16_t) (DATA_2_LEN * DATA_2_REPEAT)
#endif

#if GESTURE_MODULE_OUTPUT == 1u
#define DATA_3_PTR 			((uint8_t*)&runtime_gesture_data_arr.gestures_status)
#define DATA_3_ID 			(DEBUG_MASK + GESTURE_DEBUG_DATA_ID)
#define DATA_3_LEN			(uint16_t) (sizeof(tuneGestureData_t))
#define DATA_3_REPEAT 		1u
#define DATA_3_FRAME_LEN 	(uint16_t) (DATA_3_LEN * DATA_3_REPEAT)
#endif

/* configuration details */
static uint8_t proj_config[PROJECT_CONFIG_DATA_LEN] = {PROTOCOL_VERSION, (uint8_t) pic32cmpl, (uint8_t) TECH, (DEF_NUM_CHANNELS),
									(uint8_t) (SENSOR_NODE_CONFIG_MASK|SENSOR_KEY_CONFIG_MASK|COMMON_SENSOR_CONFIG_MASK|FREQ_HOPPING_AUTO_TUNE_MASK|SURFACE_CONFIG_MASK|GESTURE_CONFIG_MASK), (0u), (0u),
									(uint8_t) (KEY_DEBUG_MASK|FREQ_HOP_AUTO_TUNE_DEBUG_MASK|SURFACE_DEBUG_MASK|GESTURE_DEBUG_MASK), (0u),(0u)};

static uint16_t frame_len_lookup[NO_OF_CONFIG_FRAME_ID]  = {CONFIG_0_LEN,CONFIG_1_LEN,CONFIG_2_LEN,CONFIG_3_LEN,CONFIG_4_LEN,CONFIG_5_LEN,CONFIG_6_LEN};
static uint8_t *configPointerArray[NO_OF_CONFIG_FRAME_ID]	= {CONFIG_0_PTR,CONFIG_1_PTR,CONFIG_2_PTR,CONFIG_3_PTR,CONFIG_4_PTR,CONFIG_5_PTR,CONFIG_6_PTR};

/* output data details */
static uint8_t *debug_frame_PointerArray[OUTPUT_MODULE_CNT]  = {DATA_0_PTR,DATA_1_PTR,DATA_2_PTR,DATA_3_PTR};
static uint8_t debug_frame_id[OUTPUT_MODULE_CNT]		  = {DATA_0_ID,DATA_1_ID,DATA_2_ID,DATA_3_ID};
static uint16_t debug_frame_data_len[OUTPUT_MODULE_CNT]  = {DATA_0_LEN,DATA_1_LEN,DATA_2_LEN,DATA_3_LEN};
static uint16_t debug_frame_total_len[OUTPUT_MODULE_CNT] = {DATA_0_FRAME_LEN,DATA_1_FRAME_LEN,DATA_2_FRAME_LEN,DATA_3_FRAME_LEN};
static uint8_t debug_num_ch_scroller[OUTPUT_MODULE_CNT] = {DATA_0_REPEAT,DATA_1_REPEAT,DATA_2_REPEAT,DATA_3_REPEAT};
static void (*debug_func_ptr[OUTPUT_MODULE_CNT])(uint8_t ch) = {copy_run_time_data,copy_freq_hop_auto_runtime_data,copy_surface_run_time_data,copy_gesture_run_time_data};

static uint8_t frameIdToConfigID[NO_OF_CONFIG_FRAME_ID] = {PROJECT_CONFIG_ID,SENSOR_NODE_CONFIG_ID,SENSOR_KEY_CONFIG_ID,COMMON_SENSOR_CONFIG_ID,FREQ_HOPPING_AUTO_TUNE_ID,SURFACE_CONFIG_ID,GESTURE_CONFIG_ID};
...
void copy_acq_config(uint8_t channel)
{
qtm_acq_pic32cm_pl_node_config_t *ptr = &ptc_seq_node_cfg1[channel];
...
void UART_Write(uint8_t data) {
	...
	txData = data;
	if (SERCOM1_USART_Write(&txData, 1)) {
...
void touchTuneInit(void) {
    ...
    SERCOM1_USART_WriteCallbackRegister(touchUartTxComplete, touchUart);
    SERCOM1_USART_ReadCallbackRegister(touchUartRxComplete, touchUart);

    if(SERCOM1_USART_Read((void *) &rxData, 1)) {
